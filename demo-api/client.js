  ...
  async function doPay() {
    ...
    try {
      // Step 1: 402 challenge (v2 wire: PAYMENT-REQUIRED header, base64 JSON)
      log("GET /weather &rarr; expecting 402 Payment Required", "");
      setStep(1, "active");
      ...
      const pr = prHeader ? b64decode(prHeader) : await r1.json();
      const requirements = (pr.accepts && pr.accepts[0]) || pr;
      log("&larr; 402 Payment Required: " + requirements.amount + " atomic @ " + requirements.network, "");
      setStep(1, "done");

      // Step 2: EIP-3009 sign — domain derived from requirements.extra per SDK spec
      setStep(2, "active");
      ...
      const domain = {
        name: extra.name || "USDG",
        version: extra.version || "2",
        chainId: CHAIN.chainId,
        verifyingContract: requirements.asset || USDG,
      };
      const types = {
      ...
      const validBefore = now + Math.min(requirements.maxTimeoutSeconds || 3600, 3600);
      const authorization = {
        from: wallet.address,
      ...
      // Step 3: build v2 PaymentPayload and send via PAYMENT-SIGNATURE header
      setStep(3, "active");
      log("POST /weather with PAYMENT-SIGNATURE header &rarr; facilitator verify + settle...", "");
      ...
      const r2 = await fetch("/weather", {
        method: "POST",
        headers: { "Content-Type": "application/json", "payment-signature": b64encode(paymentPayload) },
      ...
      if (data.settlement?.txHash || data.settlement?.transaction) {
        document.getElementById("tx-card").style.display = "";
        const txHash = data.settlement.txHash || data.settlement.transaction;
        ...
        document.getElementById("r-to").textContent = requirements.payTo.slice(0,6) + "..." + requirements.payTo.slice(-4);
        document.getElementById("r-amt").textContent = data.paid || "0.5 USDG";
        document.getElementById("r-net").textContent = "RH Testnet (46630)";
        log("&#10003; On-chain settlement: " + txHash, "");
      }
