...
|--------|------------|--------|-------------|
| Disable all controls | `spendControls: false` | `spend_controls=False` | Any asset, no caps. |
| `maxAmountPerPayment` / `max_amount_per_payment` | `"$1"` default | `"$1"` default | USD ceiling on recognized USD-pegged assets. Set to `false`/`False` to remove the cap. |
| `allowedAssets` / `allowed_assets` | list or `true` | list or `True` | Opt into non-default tokens. Omit for default assets only; `true`/`True` to allow any asset; or a list of `{ network, asset }` entries with an optional atomic per-payment cap. |
