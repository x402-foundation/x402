...
| Option | Description |
|--------|-------------|
| `spendControls: false` | Disable all spend controls — any asset, no caps. |
| `maxAmountPerPayment` | USD ceiling on recognized USD-pegged assets. Default is `"$1"`. Set to `false` to remove the cap. |
| `allowedAssets` | Opt into non-default tokens. Omit for default assets only; `true` to allow any asset; or a list of `{ network, asset }` entries with an optional atomic `maxAmountPerPayment` per entry. |
