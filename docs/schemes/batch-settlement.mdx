...
description: "High-throughput payments with escrow, off-chain vouchers, and batched onchain redemption. Available on EVM and Solana (SVM)."
---
...
`batch-settlement` on Solana uses the same [payment-channels program](https://github.com/solana-foundation/payment-channels) as `upto`. Unlike `upto`, which closes after one metered charge, `batch-settlement` keeps the channel open across many requests and advances a cumulative watermark. The client deposits once, then signs off-chain cumulative vouchers per request; the server verifies offchain, serves immediately, and redeems accumulated vouchers later through the facilitator.

Two voucher modes are supported:

| Mode | Who signs vouchers | Per-request payload | Typical use |
|------|--------------------|---------------------|-------------|
...
In server mode the channel's onchain `authorized_signer` is the operator, who could claim the full unspent deposit. Clients enter that mode only for operator keys they have explicitly trusted out of band.

#### Server Setup (SVM)

Every channel is bound at open to a `receiverAuthorizer` key that signs cooperative close authorizations. Pass `receiverAuthorizer` to self-manage closes (recommended), or omit it to delegate to the facilitator's advertised key.
...
const batchSvmScheme = new BatchSvmScheme({
  receiverAuthorizer,
...
const resourceServer = new x402ResourceServer(facilitatorClient)
  .register("solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1", batchSvmScheme);
...
For server-signed (metered) routes, configure an `operator` signer and use `setSettlementOverrides` to charge actual usage:
...
#### Redemption Worker (SVM)

Start `BatchChannelManager` outside the request path to claim batches and distribute payouts:

```typescript
const manager = batchSvmScheme.createChannelManager(facilitatorClient, requirements, {
...
Claim promptly: if the facilitator or payer forces a close before your latest voucher is claimed, unclaimed value returns to the payer. Set `withdrawDelay` comfortably above your claim cadence.

#### Client Setup (SVM)

Register `BatchSvmScheme` for `solana:*`. The SDK opens or tops up the channel on first use and attaches the correct payload per mode:
...
facilitator.register("solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1", scheme);

// Reclaim PDA rent from abandoned channels
const rentCleanup = scheme.createRentCleanupManager("solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1");
rentCleanup.start({ intervalSecs: 60, abandonGraceSecs: 120 });
```

#### SVM Examples

* [TypeScript batch-settlement server (EVM + SVM)](https://github.com/x402-foundation/x402/tree/main/examples/typescript/servers/batch-settlement)
* [TypeScript batch-settlement client](https://github.com/x402-foundation/x402/tree/main/examples/typescript/clients/batch-settlement)
* [TypeScript batch-settlement facilitator](https://github.com/x402-foundation/x402/tree/main/examples/typescript/facilitator/batch-settlement)
