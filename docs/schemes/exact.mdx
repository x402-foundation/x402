...
### Hedera asset transfer methods

The Hedera implementation supports two asset transfer methods via `extra.assetTransferMethod`:

| Method | Description |
|--------|-------------|
| `cryptoTransfer` | Client signs a `TransferTransaction`; facilitator co-signs as fee payer. Default. Recommended for key-controlled accounts. |
| `transferExecutor` | Client supplies an authorization for a contract implementing `ITransferExecutor`; facilitator calls `executeTransfer` as fee payer. For contract-controlled funds (smart accounts, multisig/vault contracts, HIP-336 allowance delegation). |

If no `assetTransferMethod` is specified in `PaymentRequirements.extra`, clients and facilitators use `cryptoTransfer`. Facilitators advertise which methods they support per network in `/supported`:
...
For `transferExecutor`, the executor contract must implement the `ITransferExecutor` interface:
...
The client's `PaymentPayload` for `transferExecutor` carries `payer`, `executor`, and `authorization` instead of a serialized transaction:
...
Executor admission is default-deny: a facilitator must reject any executor not in its advertised `extra.executors` list. See the [`exact` Hedera spec](https://github.com/x402-foundation/x402/blob/main/specs/schemes/exact/scheme_exact_hedera.md) for full verification and settlement rules.
