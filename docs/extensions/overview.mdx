...
| [Durable Evidence](./durable-evidence) | Server + Client + Facilitator | Seals the delivered body to the parties' own keys, anchors it durably and has the facilitator sign a receipt — what was delivered, retrievable later, readable only by the parties. Buyer opt-in via `accepts`. | Rust, Python, TypeScript |
