...
Accepts a single string or an array of strings. Automatically echoes the server's `a` code and attaches the client's `s` code(s) to every payment payload. Multiple codes are useful for layered clients (e.g. an MCP server acting as middleware) that need to attribute multiple participants.
