...
These server-level hooks run for every payment regardless of which extensions are active. Extensions can also register the same hooks scoped to their own key via the `hooks` property on `ResourceServerExtension` — those run only when the extension is declared on the route. See [Extensions Overview](/extensions/overview) for details.
