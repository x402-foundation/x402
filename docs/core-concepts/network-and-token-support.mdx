...
- **Starknet**: `starknet:<chainId>` (`starknet:SN_MAIN` or `starknet:SN_SEPOLIA`, the chain id as its Cairo short string; see `specs/schemes/exact/scheme_exact_starknet.md`)

## Token Support

x402 supports tokens on EVM, Solana, TON, Algorand (AVM), Stellar, Aptos, Hedera, Keeta, NEAR, Concordium, XRPL, Cardano, and Starknet networks:
...
| **Starknet** | Any ERC-20 token (e.g., STRK) | SNIP-9 outside execution of one `transfer`, signed as SNIP-12 typed data and submitted by the facilitator |
