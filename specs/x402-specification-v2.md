...
- `deferred_until` → `success: false`, `errorReason: settlement_deferred`; `blocked` → `success: false`, `errorReason: settlement_blocked` (per §9). Neither borrows the binding's existing code for the same fact (e.g. `invalid_exact_evm_payload_authorization_valid_after`, `insufficient_funds`): those codes read as terminal to a reader that does not parse `status`, which is the one reading these states exist to prevent. The specific fact belongs in the anchor (§5.3.4): the `observation` object for `blocked`, the predicate and `statusDetail.t` for `deferred_until`.
- `expired` → `success: false`, and MAY carry the binding's existing §9 code for the same fact (e.g. `invalid_exact_evm_payload_authorization_valid_before`). `expired` is terminal, so that code's terminal reading is the correct one.
- `canceled` → `success: false`, `errorReason: authorization_canceled` (per §9). A facilitator MUST NOT substitute a nonce-consumed code, which asserts the opposite fact. The code is required rather than omitted because an absent `errorReason` does not survive the round trip: readers substitute a generic string for it, so omission renders a revoked payment indistinguishable from a failed one at the point a payer sees it.
...
**Reader obligations.** The "A reader MUST NOT conclude" column above says what a state does not prove. Two of those conclusions carry an action, and a reader holding a non-terminal `status` — `pending`, `deferred_until` or `blocked` — MUST NOT take either. It MUST NOT answer the request the authorization was presented against with a new payment required signal (§5.1), whatever that response's body says about the state: the authorization may still settle, and a fresh `PaymentRequired` obtains a *second* authorization, which the first one's replay protection is not designed to prevent and cannot. It MUST NOT present the state to the payer as a failed payment. Neither adds a requirement to the vocabulary; each states, for the party acting on a state, what the column already forbids concluding from it. How a reader resolves a non-terminal state — re-presenting the same authorization, reading the anchor — is out of scope here.
...
**5.3.3.1 Capability declarations.** A scheme/network binding that emits `status` declares, in its own scheme specification: per-state reachability — `reachable` (the binding can emit the state and meet its evidence requirement), `unreachable` (the condition cannot arise on this rail), or `unclaimable` (the condition arises and is anchorable, but the state's semantic requirement cannot be met, so the binding will not emit it) — and, for `deferred_until`, the enforcement object: the contract and field that enforce `t`, with their comparator and unit. A cell declared `unreachable` or `unclaimable` MUST carry a one-line reason. Declarations are an append-only versioned list; each entry carries `version` (monotonic per binding) and `effectiveFrom`: a CAIP-2 network plus a chain-verifiable coordinate, under an ordering rule the binding declares, at which the entry took effect. For an entry naming an enforcement object, that is the object's deployment or upgrade point; for a request-derived enforcer it resolves per-asset to the named asset contract's, under the predicate the declaration states. An entry naming no enforcement object — e.g. one declaring `deferred_until` `unreachable` — takes effect where the binding says; the first such entry MAY name the binding's genesis-equivalent. On a rail with a global block height the coordinate is that height. In every case it is something consensus produced and a reader can confirm, never a value the emitter asserts. A reader selects the entry whose `[effectiveFrom, next.effectiveFrom)` covers, under that ordering rule, the anchor's coordinate: for a settlement, the settling transaction's; for a non-settlement state, the ordering coordinate its anchor declares where it declares one, and otherwise the entry in effect at the head as of `statusAnchor.observedAt`.
...
- **`authorization_canceled`**: The authorization was revoked by the party the binding names, and can no longer settle. Facilitators MAY return this **terminal** code; it accompanies `status: "canceled"` (§5.3.3) wherever `status` is emitted. It is distinct from a nonce-consumed code, which asserts that the payer spent rather than revoked, and it exists because cancellation is the one outcome the existing codes cannot express: an omitted `errorReason` is replaced with a generic string by the time it reaches a payer, so revocation and failure become indistinguishable.
- **`settlement_deferred`**: The authorization is valid, but its earliest-settlement bound has not passed; it can settle later without the payer signing again. Facilitators MAY return this **non-terminal** code; it accompanies `status: "deferred_until"` (§5.3.3). It is not a request for a new authorization — the first may still settle, and a second one can settle alongside it.
- **`settlement_blocked`**: The authorization is live, but settlement cannot proceed at the point observed — for example, insufficient balance or allowance — and it may settle later if that changes, without the payer signing again. Facilitators MAY return this **non-terminal** code; it accompanies `status: "blocked"` (§5.3.3). It is not a request for a new authorization, for the same reason.

Where a code accompanies a `status` (§5.3.3), the two agree on terminality by construction: each non-terminal state carries a non-terminal code above; `canceled` carries `authorization_canceled`; `expired` MAY carry the binding's terminal expiry code; and `settled` is `success: true`. A reader that does not parse `status` therefore reaches the same terminality from the code alone. Responses without `status` are unaffected.
