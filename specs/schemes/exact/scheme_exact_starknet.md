...
The scheme has a single asset transfer method, `"default"`, in the [facilitator-submitted](./scheme_exact.md#facilitator-submitted) family: the client signs a SNIP-9 `OutsideExecution` and the sponsor executes it, so the client has nothing to choose. `extra.assetTransferMethod` MAY be omitted, or set explicitly to `"default"`.
...
> **Selector-encoding note:** `Selector` is the felt `sn_keccak("transfer")` and MUST enter the SNIP-12 hash as that felt, not be hashed again as a name.
...
All verification state reads and simulations MUST use an accepted-block identifier (`latest`); pre-confirmed state MUST NOT feed any accept-or-settle decision, though a facilitator MAY observe pre-confirmed status while waiting for confirmation. Receipt and transaction lookups carry no block identifier — for those the equivalent requirement is the finality gate of settlement step 3 (an accepted `finality_status` plus a `block_number`). RPC 0.9 renamed the `pending` block tag to `pre_confirmed` (current nodes reject `pending`), and reading confirmed state is the safe default in any case: its staleness costs a revert (gas), never funds, since a revert rolls back the SNIP-9 nonce.
...
1. On `/settle`, reject immediately with `duplicate_settlement` if the key is present, unless it is the retry of the same payload that settlement step 4 waits on.
...
The pre-broadcast dedup cache is an optimization only; the SNIP-9 nonce remains the authoritative replay protection. The observable requirement in settlement step 4 — never re-broadcasting an authorization whose outcome is unresolved — is normative regardless of how it is tracked. A per-process cache does not protect horizontally scaled facilitators — those need a shared store, or accept the residual race (safe onchain; only gas is at stake).
