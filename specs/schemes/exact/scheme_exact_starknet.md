...
| `settlement_pending` | broadcast outcome not yet terminal — **non-terminal**, `success: false`. Carries the transaction hash when the broadcast produced one, so the caller reconciles it onchain; when the broadcast itself timed out with no hash, the `transaction` is empty and the caller retries after the guard window |
