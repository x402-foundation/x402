...
| ----- | ------------ | -------------------- |
| `settled` | `reachable` | `composite`, or `event` where the join abstains (below): `AuthorizationUsed(address indexed authorizer, bytes32 indexed nonce)` joined to an ERC-20 `Transfer` emitted by the token named in `PaymentRequirements.asset`, in the same transaction, with `from == authorizer`. Join rule: the pair `(transaction, authorizer)` attributes the movement only where it is unambiguous — where exactly one such `Transfer` qualifies. Where the transaction carries more than one, because it settles two authorizations from the same payer in that asset, the join is ambiguous: a facilitator MUST NOT attribute a movement to a nonce, and MUST NOT emit a `composite` whose `Transfer` part is unresolved or paired by position, since §5.3.4 requires every part to resolve. It anchors `settled` as `kind: event` on the `AuthorizationUsed` log alone, which §5.3.3 permits and which still proves execution: EIP-3009 marks the nonce used and performs the transfer within one call, so the log survives only in a receipt where the transfer executed. `amount` is then the authorization's `value` rather than a figure read from a `Transfer` log. Log adjacency MUST NOT be used to disambiguate — emitting the event before the transfer is a property of the deployed token, not a requirement of EIP-3009. `about`: `{authorizer, nonce}`. |
...
| `canceled` | `reachable` | `AuthorizationCanceled(address indexed authorizer, bytes32 indexed nonce)` — emitted only by a successful cancellation of an unused nonce, so conclusive alone. `authorizationState()` MUST NOT be used to attribute: `true` means used **or** canceled. `statusDetail.by`: `authorizer` — the address the event indexes, which for EIP-3009 is the payer. |
