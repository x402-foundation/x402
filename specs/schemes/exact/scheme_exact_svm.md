...
- **Allowance resolves and covers the draw (MUST).** Before signing, the sponsor MUST read the cited delegation account on-chain and confirm it exists, is not expired, its delegator, spender, and mint match the draw, and its remaining cap is `>= amount`, fail-closed, at the same status as the §1 outcome check. The program enforces the cap and expiry at execution, but `verify()` MUST remain a sound predictor of `settle()`: a sponsor MUST NOT sign a draw whose allowance does not resolve and cover it (`allowance_missing`, `allowance_expired`, `allowance_under_cap`, and `allowance_mismatch` for a resolvable, active, funded allowance whose delegator, spender, or mint does not match the draw).

For settlement-receipt binding, the delegation is the authorization artifact and the executed `transferFixed` is the finalized result. A receipt, when produced per #2666, MUST bind the facilitator's verified net-balance result to `ATA(payTo)`, recomputable from the recipient ATA's post-balances and keyed to the executed settlement id from `PAYMENT-RESPONSE`, with the delegation as provenance rather than the delegation itself, which proves authority, not execution. This carries the same normative strength as the other `exact` receipt-binding vectors.
