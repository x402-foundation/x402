...
| Cardano route `extra` and payees | Cardano payload semantics live with the scheme registration in [`servers/typescript/config.ts`](servers/typescript/config.ts), not in the catalog: the `script` route pays the always-succeeds validator's address and declares the script + inline datum, and the `masumi` route is a *template* (`extra.assetTransferMethod: "masumi"`, `payTo` = the escrow address) that `ExactCardanoScheme` turns into a fresh seller-signed quote per 402 and matches back on the paid retry. No client, server or transport overlay excludes Cardano; MCP works because the scheme issues inside `createPaymentRequiredResponse`, which the MCP wrapper calls per tool call. `CARDANO_L1_CONFIRMATIONS` (unset = spec default of one confirmation) picks the confirmation policy for all three routes and, at `-1`, lets the facilitator settle on its own broadcast acceptance. |
...
CARDANO_L1_CONFIRMATIONS=           # Optional confirmation policy for the Cardano routes (-1..20). Unset = spec default (1 confirmation, ~40s/payment on preprod); -1 = settle on the facilitator's own broadcast acceptance
SERVER_CARDANO_SELLER_MNEMONIC=     # Optional Masumi selling wallet (signs the quotes, needs no funds); a well-known test phrase when unset
...
2. Fund the **client** wallet with test ADA from the [Cardano testnets faucet](https://docs.cardano.org/cardano-testnets/tools/faucet/) (select **Preprod**); ~10 tADA is plenty. Only the client needs funds — it builds, signs, and pays the fee. For the stablecoin paths, get preprod **tUSDM** from the [tUSDM faucet](https://tusdm.moneta.global).
...
> **Note:** Every Cardano scenario runs with the confirmation policy real users get by default — one confirmation past block inclusion, about 40s on preprod. When a block gap outlasts one settle attempt (75s), the facilitator reports `settlement_pending` and the resource server retries once; the harness log shows this as a `⏳ Settlement pending` line. Set `CARDANO_L1_CONFIRMATIONS=-1` to settle on the facilitator's own broadcast acceptance (a few seconds per scenario) when iterating locally; the facilitator accepts mempool evidence only when that variable is exactly `-1`.
