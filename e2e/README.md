...
> **Note:** Every Cardano scenario runs with the confirmation policy real users get by default — one confirmation past block inclusion, about 40s on preprod — and the facilitator reports `settlement_pending` in between, which the resource server retries once. Set `CARDANO_L1_CONFIRMATIONS=-1` to settle on the facilitator's own broadcast acceptance (a few seconds per scenario) when iterating locally; the facilitator accepts mempool evidence only when that variable is exactly `-1`.
