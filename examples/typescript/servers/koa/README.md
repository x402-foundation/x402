# @x402/koa Example Server

Koa server demonstrating how to protect API endpoints with a paywall using the `@x402/koa` middleware.
...
app.use(async ctx => {
  if (ctx.path === "/weather") {
    ctx.body = { weather: "sunny", temperature: 70 };
  }
});
```
