...
// Coinbase Facilitator requires a JSON Web Key (JWK) for authentication.
// We handle this requirement by adding a hook that runs ontop of the facilitator to create a JWT to pass along in the header.
fn get_cdp_request_hook() -> Arc<CoinbaseRequestHook> {
    ...
    CoinbaseRequestHook::new(&api_key, &api_secret, None, None, Some(true))
        .expect("Cdp request hook failed to generate")
}
...
fn get_x402_config() -> X402Config {
    ...
    let price_usdc = parse_units("0.01", 6).expect("Failed to parse price units");
    ...
    // Since we need to pass a request hook into the facilitator, we create one
...
async fn main() {
    ...
    let app = Router::new()
        .route("/api/premium", get(premium_resource_endpoint))
