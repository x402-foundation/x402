 ...
 */
export const getDefaultAsset: GetDefaultAsset<CasperDefaultAsset> = (network, symbol?) => {
 ...
 */
export const findDefaultAsset: FindDefaultAsset<CasperDefaultAsset> = (asset, network) => {
  const assets = DEFAULT_ASSETS[network];
  if (!assets) {
    return undefined;
  }
