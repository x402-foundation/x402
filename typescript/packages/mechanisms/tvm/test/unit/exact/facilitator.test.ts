...
import { ERR_SETTLEMENT_FAILED } from '../../../src/exact/facilitator/errors'
...
describe('ExactTvmScheme (Facilitator)', () => {
  ...
  function mockFetchForVerifyAndSettle(
    verifyResponse: Record<string, unknown> = { isValid: true, payer: '0:payer' },
  ...
  describe('getExtra', () => {
    it('should advertise sponsored fees for supported networks', () => {
      const extra = facilitator.getExtra(TVM_MAINNET)
      expect(extra).toEqual({ areFeesSponsored: true })
    })
  ...
  describe('verify', () => {
    ...
    it('should include payer address in response', async () => {
      ...
      const payload = makeValidPayload()
      const result = await facilitator.verify(payload, validRequirements)
      expect(result.payer).toBe('0:payer')
    })
  })
  ...
  describe('settle', () => {
    ...
    it('should handle /settle endpoint failure', async () => {
      vi.spyOn(global, 'fetch').mockImplementation(async (input: RequestInfo | URL) => {
        ...
        return new Response(JSON.stringify({ success: false, errorReason: 'insufficient_gas' }), {
          status: 200,
        })
      })
