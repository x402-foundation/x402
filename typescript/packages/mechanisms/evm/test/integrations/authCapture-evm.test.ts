...
import { AuthCaptureEvmScheme as AuthCaptureEvmServer } from "../../src/authCapture/server/scheme";
import { AuthCaptureEvmScheme as AuthCaptureEvmFacilitator } from "../../src/authCapture/facilitator/scheme";
import type { AuthCaptureExtra } from "../../src/authCapture/types";
...
if (!HAS_KEYS) {
  console.warn(
    "[authCapture-evm.test.ts] Skipping on-chain tests: CLIENT_PRIVATE_KEY and FACILITATOR_PRIVATE_KEY env vars are required.",
  );
}
...
/**
 * Builds payment requirements for the authCapture scheme on Base Sepolia.
 *
  ...
  describeOnChain("x402Client / x402ResourceServer / x402Facilitator - direct API", () => {
    ...
    it(
      ...
      async () => {
        ...
        const resource = {
          ...
          description: "authCapture test resource",
        ...
        expect(payload.accepted.scheme).toBe("authCapture");
    ...
    it(
      ...
      async () => {
        ...
        const resource = {
          url: "https://example.com/api",
          description: "authCapture charge test",
          mimeType: "application/json",
        };
    ...
    it(
      ...
      async () => {
        ...
        const resource = {
          url: "https://example.com/api",
          description: "authCapture Permit2 test",
          mimeType: "application/json",
        };
    ...
    it(
      ...
      async () => {
        ...
        const resource = {
          url: "https://example.com/api",
          description: "authCapture Permit2 charge test",
          mimeType: "application/json",
        };
