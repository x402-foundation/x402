...
export type AptosDefaultAsset = DefaultAsset;

/** Default USD-pegged assets by CAIP-2 network; index 0 is the `"$0.10"` default. */
export const DEFAULT_ASSETS: DefaultAssetTable<AptosDefaultAsset> = {
  [APTOS_MAINNET_CAIP2]: [{ asset: USDC_MAINNET_FA, decimals: 6, symbol: "USDC" }],
  [APTOS_TESTNET_CAIP2]: [{ asset: USDC_TESTNET_FA, decimals: 6, symbol: "USDC" }],
};
 ...
 */
export const getDefaultAsset: GetDefaultAsset<AptosDefaultAsset> = (network, symbol?) => {
 ...
 *
 * @param asset - FA metadata address from payment requirements
 * @param network - CAIP-2 network
 * @returns Matching entry, or undefined
 */
export const findDefaultAsset: FindDefaultAsset<AptosDefaultAsset> = (asset, network) => {
  const assets = DEFAULT_ASSETS[network];
  if (!assets) {
    return undefined;
  }
