...
import { BatchSettlementHederaScheme } from "../../../src/batch-settlement/server/scheme";
...
import { registerBatchSettlementDeployment } from "../../../src/batch-settlement/constants";
import {
  keyTypeOf,
  resolveSigningKeyFromKey,
  signDigestWithPrivateKey,
  verifyDigestSignature,
} from "../../../src/batch-settlement/signing";
import type { ClientHederaBatchSigner } from "../../../src/batch-settlement/signer";

const PAYER_KEY = PrivateKey.fromStringECDSA(
  "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d",
);

vi.mock("../../../src/batch-settlement/signing", async importOriginal => {
  const original = await importOriginal<typeof import("../../../src/batch-settlement/signing")>();
  return {
    ...original,
    createAccountKeyResolver: () => async (account: string) => {
      if (account.toLowerCase() === "0x70997970C51812dc3A010C7d01b50e0d17dc79C8".toLowerCase()) {
        return original.resolveSigningKeyFromKey(PAYER_KEY.publicKey);
      }
      return { ok: false, reason: "account_not_found", message: "unknown test account" };
    },
  };
});
import * as Errors from "../../../src/batch-settlement/errors";

function buildManager(scheme: BatchSettlementHederaScheme): BatchSettlementChannelManager {
...
async function runBeforeVerify(
  server: BatchSettlementHederaScheme,
...
registerBatchSettlementDeployment(NETWORK, {
  settlement: "0x00000000000000000000000000000000009F0001",
  settlementId: "0.0.10420225",
  collector: "0x00000000000000000000000000000000009F0002",
...
function buildAuthorizerSigner(): AuthorizerSigner {
  return {
    address: PAYER,
    keyType: keyTypeOf(PAYER_KEY),
    signDigest: digest => signDigestWithPrivateKey(PAYER_KEY, digest),
  };
}

function buildPayerSigner(): ClientHederaBatchSigner {
  const resolved = resolveSigningKeyFromKey(PAYER_KEY.publicKey);
  return {
    accountId: "0.0.7001",
    evmAddress: PAYER,
    keyType: keyTypeOf(PAYER_KEY),
...
async function buildSignedVoucherPayload(
  ...
  const voucher = await signVoucher(buildPayerSigner(), channelId, maxClaimableAmount, NETWORK);
...
describe("BatchSettlementHederaScheme — construction", () => {
  it("uses an in-memory channel storage by default", () => {
    const server = new BatchSettlementHederaScheme(RECEIVER);
  ...
  it("allows enforceMinDeposit to be enabled", () => {
    const server = new BatchSettlementHederaScheme(RECEIVER, { enforceMinDeposit: true });
...
describe("BatchSettlementHederaScheme — validateFacilitatorSupport", () => {
...
describe("BatchSettlementHederaScheme — onBeforeVerify", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;

  beforeEach(() => {
    storage = new InMemoryChannelStorage();
    server = new BatchSettlementHederaScheme(RECEIVER, { storage });
  });
  ...
  it("rejects deposits below minDeposit when enforcement is enabled", async () => {
    const enforcingServer = new BatchSettlementHederaScheme(RECEIVER, {
      storage,
  ...
  it("reuses the mismatch channel snapshot for corrective payment-required accepts", async () => {
    ...
    const snapshotServer = new BatchSettlementHederaScheme(RECEIVER, { storage: countingStorage });
...
describe("BatchSettlementHederaScheme — two-phase reservation (security)", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;
  const validResult = {
    isValid: true,
...
describe("BatchSettlementHederaScheme — pending cleanup hooks", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;
...
describe("BatchSettlementHederaScheme — onAfterVerify", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;
...
describe("BatchSettlementHederaScheme — onBeforeSettle", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;
...
describe("BatchSettlementHederaScheme — onAfterSettle", () => {
  let server: BatchSettlementHederaScheme;
  let storage: InMemoryChannelStorage;
...
describe("BatchSettlementChannelManager — refund pending channels", () => {
  it("skips channels with live pending requests", async () => {
    const storage = new InMemoryChannelStorage();
    const server = new BatchSettlementHederaScheme(RECEIVER, { storage });
    const facilitator = {
      settle: vi.fn(async () => ({
        success: true,
        transaction: "0xref",
