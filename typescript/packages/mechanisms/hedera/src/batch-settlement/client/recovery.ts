...
export async function recoverFromSignature(
  ...
  // The client verifies the server-returned voucher against its own voucher signing key. When
  // `payerAuthorizer` points at a key this client does not hold, the proof cannot be checked.
  const voucherSigner = deps.voucherSigner ?? deps.signer;
  const expectedSigner = getAddress(deps.payerAuthorizer ?? voucherSigner.evmAddress);
  if (expectedSigner.toLowerCase() !== getAddress(voucherSigner.evmAddress).toLowerCase()) {
    return false;
  }
  const digest = hashTypedData({
    domain: getBatchSettlementEip712Domain(accept.network),
    types: voucherTypes,
    primaryType: "Voucher",
    message: {
      channelId,
  ...
  if (!(await voucherSigner.verifyOwnSignature(digest, sig))) {
    return false;
  }
