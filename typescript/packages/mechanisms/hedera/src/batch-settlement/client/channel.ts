...
import { MIN_WITHDRAW_DELAY, getBatchSettlementDeployment } from "../constants";
import { resolveReceiverAddress, resolveTokenAddress } from "../addresses";
import type {
...
export function buildChannelConfig(
  ...
  const receiver = resolveReceiverAddress(paymentRequirements.payTo);
  if (!receiver) {
    throw new Error(
      `Payment requirements payTo must be a Hedera account id: ${paymentRequirements.payTo}`,
    );
  }
  const token = resolveTokenAddress(paymentRequirements.asset);
  if (!token) {
    throw new Error(
      `Payment requirements asset must be an HTS token id: ${paymentRequirements.asset}`,
    );
  }

  return {
    payer: getAddress(deps.signer.evmAddress),
    payerAuthorizer: getAddress(
      deps.payerAuthorizer ?? deps.voucherSigner?.evmAddress ?? deps.signer.evmAddress,
    ),
    receiver,
...
export async function recoverChannel(
  ...
  if (!deps.signer.readContract) {
    throw new Error("recoverChannel requires ClientHederaBatchSigner.readContract");
  }
...
/**
 * Reads `channels(channelId)` returning `[balance, totalClaimed]` from the escrow deployed on `network`.
 *
...
export async function readChannelBalanceAndTotalClaimed(
  ...
  if (!signer.readContract) {
    throw new Error(
      "readChannelBalanceAndTotalClaimed requires ClientHederaBatchSigner.readContract",
    );
  }
  return (await signer.readContract({
    address: getAddress(getBatchSettlementDeployment(network).settlement),
