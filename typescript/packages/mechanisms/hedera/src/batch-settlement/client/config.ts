...
/**
 * Full options object accepted by `BatchSettlementHederaScheme`. Either this or a
 * bare {@link BatchSettlementDepositPolicy} can be passed as the second
 * constructor argument.
 */
export interface BatchSettlementHederaSchemeOptions {
  ...
  /**
   * When set, vouchers are signed with this account key and its EVM address is committed as
   * `payerAuthorizer`; deposit authorizations still use the main `signer` (the payer).
   */
 ...
 * @returns `true` when `o` is a {@link BatchSettlementHederaSchemeOptions} object.
 */
export function isBatchSettlementHederaSchemeOptions(
  o: BatchSettlementHederaSchemeOptions | BatchSettlementDepositPolicy | undefined,
): o is BatchSettlementHederaSchemeOptions {
  return (
    o !== undefined &&
    typeof o === "object" &&
    ("storage" in o ||
...
export function resolveClientOptions(
  second?: BatchSettlementHederaSchemeOptions | BatchSettlementDepositPolicy,
): ResolvedClientOptions {
  if (second === undefined) {
    return { storage: new InMemoryClientChannelStorage(), salt: DEFAULT_SALT };
  }
  if (isBatchSettlementHederaSchemeOptions(second)) {
    return {
