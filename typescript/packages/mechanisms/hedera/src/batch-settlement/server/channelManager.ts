...
export interface ChannelManagerConfig {
  ...
  /** Receiver EVM address (long-zero form of the server's `payTo` account). */
  receiver: `0x${string}`;
  /** Token EVM address (long-zero form of the HTS token id). */
  token: `0x${string}`;
  network: Network;
  /** Server account id (`0.0.x`) advertised as `payTo`; defaults to `receiver`. */
  payTo?: string;
  /** HTS token id (`0.0.x`) advertised as `asset`; defaults to `token`. */
  asset?: string;
}
...
export class BatchSettlementChannelManager {
  private readonly scheme: BatchSettlementHederaScheme;
  ...
  constructor(config: ChannelManagerConfig) {
    ...
    this.payTo = config.payTo ?? config.receiver;
    this.asset = config.asset ?? config.token;
  }
