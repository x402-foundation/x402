...
import { createAccountKeyResolver, type AccountKeyResolver } from "../signing";
import { entityIdToLongZeroAddress, tokenIdToEvmAddress } from "../addresses";
...
export interface BatchSettlementHederaSchemeServerConfig {
  ...
  /**
   * Mirror Node base URL used to resolve payer keys for local voucher verification.
   * Defaults to the public Mirror Node of the request's network.
   */
  mirrorNodeUrl?: string;
  /**
   * Verify plain vouchers locally (Mirror Node key lookup + off-chain signature check) when the
   * mirrored onchain state is fresh, instead of always calling the facilitator. Default true.
   */
  localVoucherVerification?: boolean;
  /** Custom account key resolver for local voucher verification (defaults to a cached Mirror Node resolver). */
...
/**
 * Server-side implementation of the `batch-settlement` scheme for Hedera networks.
 *
 * `receiverAddress` is the long-zero EVM address of the server's `payTo` account; route
 * `payTo` values stay Hedera account ids (`0.0.x`).
 */
export class BatchSettlementHederaScheme implements SchemeNetworkServer {
  readonly scheme = BATCH_SETTLEMENT_SCHEME;
  readonly defaultAssetTransferMethod: BatchSettlementAssetTransferMethod =
    HTS_ALLOWANCE_TRANSFER_METHOD;
  readonly paymentFlows = {
    [HTS_ALLOWANCE_TRANSFER_METHOD]: { supported: ["authorization"], default: "authorization" },
  ...
  private readonly mirrorNodeUrl: string | undefined;
  private readonly localVoucherVerification: boolean;
  private readonly accountKeyResolver: AccountKeyResolver | undefined;
  private readonly keyResolvers = new Map<string, AccountKeyResolver>();

  /**
   * Constructs a batched server scheme.
   *
   * @param receiverAddress - The server's receiver: a Hedera account id (`0.0.x`) or its long-zero EVM address.
   * @param config - Optional configuration for storage, receiver-authorizer signer, and withdraw delay.
   */
  constructor(
    receiverAddress: `0x${string}` | string,
    config?: BatchSettlementHederaSchemeServerConfig,
  ) {
    this.receiverAddress = HEDERA_ENTITY_ID_REGEX.test(receiverAddress)
      ? entityIdToLongZeroAddress(receiverAddress)
      : getAddress(receiverAddress);
    this.mirrorNodeUrl = config?.mirrorNodeUrl;
    this.localVoucherVerification = config?.localVoucherVerification ?? true;
    this.accountKeyResolver = config?.accountKeyResolver;
   ...
   */
  registerMoneyParser(parser: MoneyParser): BatchSettlementHederaScheme {
  ...
  async parsePrice(price: Price, network: Network): Promise<AssetAmount> {
    ...
    if (typeof price === "object" && price !== null && "amount" in price) {
      if (!price.asset || !HEDERA_ENTITY_ID_REGEX.test(price.asset)) {
        throw new Error(`AssetAmount.asset must be an HTS token id (0.0.x) on network ${network}`);
      }
      return {
        amount: price.amount,
        asset: price.asset,
        extra: { assetTransferMethod: HTS_ALLOWANCE_TRANSFER_METHOD, ...(price.extra || {}) },
      };
    }
  ...
  async enhancePaymentRequirements(
    ...
    if (!HEDERA_ENTITY_ID_REGEX.test(paymentRequirements.payTo)) {
      throw new Error(
        `batch-settlement payTo must be a Hedera account id: ${paymentRequirements.payTo}`,
      );
    }
    if (
      !HEDERA_ENTITY_ID_REGEX.test(paymentRequirements.asset) ||
      paymentRequirements.asset === "0.0.0"
    ) {
      throw new Error(
        `batch-settlement asset must be an HTS token id: ${paymentRequirements.asset}`,
      );
    }
   ...
   *
   * @returns `true` when {@link BatchSettlementHederaSchemeServerConfig.enforceMinDeposit} is enabled.
   */
  ...
  createChannelManager(
    ...
    const tokenId = token ?? getDefaultAsset(network).asset;
    if (!HEDERA_ENTITY_ID_REGEX.test(tokenId)) {
      throw new Error(`createChannelManager token must be an HTS token id: ${tokenId}`);
    }
    return new BatchSettlementChannelManager({
      scheme: this,
      facilitator,
      receiver: this.receiverAddress,
      token: tokenIdToEvmAddress(tokenId),
      network,
      asset: tokenId,
    });
  }
  ...
  /**
   * Returns the cached Mirror Node account key resolver used for local voucher verification,
   * or `undefined` when local verification is disabled.
   *
   * @param network - CAIP-2 Hedera network of the request.
   * @returns Resolver, or `undefined`.
   */
  getAccountKeyResolver(network: Network): AccountKeyResolver | undefined {
    if (!this.localVoucherVerification || !isSupportedHederaNetwork(network)) {
      return undefined;
    }
    if (this.accountKeyResolver) {
      return this.accountKeyResolver;
    }
    let resolver = this.keyResolvers.get(network);
    if (!resolver) {
      resolver = createAccountKeyResolver({
        mirrorNodeUrl: this.mirrorNodeUrl ?? mirrorNodeUrlForNetwork(network),
      });
      this.keyResolvers.set(network, resolver);
    }
    return resolver;
  }
