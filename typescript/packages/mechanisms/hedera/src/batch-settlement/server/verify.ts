...
import { verifyDigestSignature } from "../signing";
import { createDepositNonce } from "../client/hederaAllowance";
...
import type { BatchSettlementHederaScheme } from "./scheme";
 ...
 *
 * @param scheme - Owning `BatchSettlementHederaScheme` instance for storage access.
...
> {
  ...
  try {
    ...
    const now = Date.now();
    const pendingId = createDepositNonce();
 ...
 *
 * @param scheme - Owning `BatchSettlementHederaScheme` instance.
...
async function verifyVoucherLocally(
  ...
  const signatureOk = await verifyLocalVoucherSignature(scheme, raw, requirements.network);
  if (signatureOk === undefined) {
    // Key could not be resolved locally; let the facilitator verify.
    return;
  }
  if (!signatureOk) {
...
/**
 * Verifies the voucher signature against the payer authorizer's Hedera account key, resolved
 * (and cached) through the Mirror Node.
 *
 * @param scheme - Batch settlement scheme providing the account key resolver.
 * @param raw - Decoded batch-settlement voucher payload.
 * @param network - Hedera network identifier for the EIP-712 domain.
 * @returns Whether the signature is valid, or `undefined` when the key cannot be resolved
 *   locally (caller falls back to facilitator verification).
 */
async function verifyLocalVoucherSignature(
  ...
  const resolver = scheme.getAccountKeyResolver(
    network as Parameters<typeof scheme.getAccountKeyResolver>[0],
  );
  if (!resolver) {
    return undefined;
  }
  try {
    const resolution = await resolver(getAddress(raw.channelConfig.payerAuthorizer));
    if (!resolution.ok) {
      return resolution.reason === "unsupported_key" ? false : undefined;
    }
    ...
    return await verifyDigestSignature({
      key: resolution.key,
      digest,
      signature: raw.voucher.signature,
    });
