 ...
 * (`extra.recentBlockhash` + `extra.lastValidBlockHeight`) — mirroring how MPP
 * embeds a `recentBlockhash` — so the client needn't make its own RPC
 * round-trip. Falls back to fetching one from `rpc` when the challenge omits it.
 *
