...
/// @title x402BatchSettlementHedera
/// @notice Stateless unidirectional payment channel contract for the x402 `batch-settlement` scheme on Hedera.
///
/// @dev Hedera binding of `x402BatchSettlement`. Channel identity is derived from an immutable `ChannelConfig`:
///      `channelId = EIP712Hash(channelConfig)` bound to `block.chainid` (295 mainnet / 296 testnet) and this address.
///
///      Differences from the EVM contract:
///      - Every signature (payer voucher, `receiverAuthorizer` claim batch and refund) is verified through the
///        Hedera Account Service system contract (HIP-632 `isAuthorizedRaw`) so both ED25519 (64-byte) and ECDSA
///        secp256k1 (65-byte `r || s || v`) Hedera account keys are accepted. Signers are Hedera accounts identified
///        by their EVM address (long-zero or EVM alias); there is no EIP-1271 path.
///      - `associateToken` lets anyone associate this escrow with an HTS token so it can hold e.g. USDC.
///      - Deposits are pulled by `HederaAllowanceDepositCollector` using HTS allowances (no Permit2 / ERC-3009).
///
///      Uses `ReentrancyGuardTransient` (EIP-1153); Hedera supports Cancun transient storage (HIP-865).
///      Fee-on-transfer and rebasing tokens are not recommended and are not guaranteed to behave like standard ERC-20s.
///
/// @author x402 Foundation (derived from the Coinbase EVM contract)
contract x402BatchSettlementHedera is EIP712, Multicall, ReentrancyGuardTransient {
    ...
    /// @notice Emitted when the escrow associates itself with an HTS token (`responseCode` 22 or 194).
    ...
    /// @notice HTS association returned a failure response code.
    ...
    /// @notice Associates this escrow contract with an HTS token so it can receive and hold it.
    ///
    /// @dev Permissionless: association is a prerequisite for any HTS deposit and cannot harm the contract.
    ///      Succeeds (idempotently) when the token is already associated. Reverts with the HTS response code otherwise.
    ///
    /// @param token The HTS token EVM address (long-zero form of the token id).
    function associateToken(address token) external nonReentrant {
        if (token == address(0)) revert InvalidChannel();
        int64 rc = HederaSystemContracts.associateSelf(token);
        if (rc != HederaSystemContracts.SUCCESS && rc != HederaSystemContracts.TOKEN_ALREADY_ASSOCIATED_TO_ACCOUNT) {
            revert TokenAssociationFailed(rc);
        }
    ...
    function claimWithSignature(
        ...
        if (!HederaSystemContracts.isAuthorizedRaw(authorizer, digest, authorizerSignature)) {
    ...
    function refundWithSignature(
        ...
        if (!HederaSystemContracts.isAuthorizedRaw(config.receiverAuthorizer, digest, receiverAuthorizerSignature)) {
            revert InvalidSignature();
        }
        _executeRefund(config, amount);
    }
    ...
    function _processVoucherClaim(
        ...
        // Vouchers are signed by the committed `payerAuthorizer` account when set, otherwise by the payer
        // account itself. Both are Hedera accounts; HAS validates ED25519 or ECDSA raw signatures.
        address payerAuth = vc.voucher.channel.payerAuthorizer;
        address voucherSigner = payerAuth != address(0) ? payerAuth : vc.voucher.channel.payer;
        if (!HederaSystemContracts.isAuthorizedRaw(voucherSigner, digest, vc.signature)) {
            revert InvalidSignature();
        }
