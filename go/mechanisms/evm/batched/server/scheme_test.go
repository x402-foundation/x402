...
func TestGetClaimableVouchers_NoSessions(t *testing.T) {
	s := NewBatchedEvmScheme("0xreceiver", nil)
	got, err := s.GetClaimableVouchers(nil)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if len(got) != 0 {
...
func TestGetClaimableVouchers_FiltersByIdle(t *testing.T) {
	...
	_ = s.UpdateSession("0xa", sess)
	got, _ := s.GetClaimableVouchers(&GetClaimableVouchersOpts{IdleSecs: 3600})
	if len(got) != 0 {
...
func TestGetWithdrawalPendingSessions(t *testing.T) {
	...
	got, err := s.GetWithdrawalPendingSessions()
	if err != nil {
		t.Fatalf("err: %v", err)
	}
