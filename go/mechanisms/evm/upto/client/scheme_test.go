...
func uptoPermit2Requirements() types.PaymentRequirements {
...
func captureUptoLogs(t *testing.T) *bytes.Buffer {
	t.Helper()
...
func TestCreatePaymentPayloadWarnsOnceWhenEip2612CapabilityIsMissing(t *testing.T) {
	logs := captureUptoLogs(t)
	scheme := NewUptoEvmScheme(&mockMinimalClientSigner{}, nil)
	...
	for range 2 {
		payload, err := scheme.CreatePaymentPayload(context.Background(), uptoPermit2Requirements(), ctx)
		if err != nil {
	...
	output := logs.String()
	if count := strings.Count(output, "[x402 upto] eip2612GasSponsoring"); count != 1 {
...
func TestCreatePaymentPayloadDoesNotWarnWhenExtensionIsNotAdvertised(t *testing.T) {
	...
	if _, err := scheme.CreatePaymentPayload(context.Background(), uptoPermit2Requirements(), x402.PaymentPayloadContext{}); err != nil {
		t.Fatalf("CreatePaymentPayload failed: %v", err)
	}
	if logs.Len() != 0 {
...
func TestCreatePaymentPayloadWarnsWhenErc20ApprovalCapabilityIsMissing(t *testing.T) {
	...
	output := logs.String()
	if !strings.Contains(output, "[x402 upto] erc20ApprovalGasSponsoring") ||
