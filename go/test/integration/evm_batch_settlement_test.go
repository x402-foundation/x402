...
import (
	...
	batchedfacilitator "github.com/x402-foundation/x402/go/mechanisms/evm/batched/facilitator"
...
// server scheme; SignTypedData) and batched.AuthorizerSigner (for the facilitator
...
func (s *batchedAuthorizerSigner) SignRefund(
	...
	domain := batched.GetBatchSettlementEip712Domain(chainId)
	allTypes := map[string][]evmmech.TypedDataField{"Refund": batched.RefundTypes["Refund"]}
...
type batchedPipeline struct {
	clientScheme      *batchedclient.BatchedEvmScheme
	serverScheme      *batchedserver.BatchedEvmScheme
	facilitatorScheme *batchedfacilitator.BatchedEvmScheme
...
func buildBatchedPipeline(t *testing.T, keys *batchedTestKeys) *batchedPipeline {
	...
	facilitatorScheme := batchedfacilitator.NewBatchedEvmScheme(facilitatorSigner, authorizerSigner)
	...
	serverScheme := batchedserver.NewBatchedEvmScheme(keys.receiver, &batchedserver.BatchedEvmSchemeConfig{
...
func onChainChannel(ctx context.Context, signer *realFacilitatorEvmSigner, channelId string) (*big.Int, *big.Int, error) {
	...
	result, err := signer.ReadContract(ctx, batched.BatchSettlementAddress, batched.BatchSettlementChannelsABI, "channels", channelIdBytes)
	if err != nil {
		return nil, nil, err
	}
...
func (p *batchedPipeline) channelIdForRequirements(req types.PaymentRequirements) string {
	...
	return batched.NormalizeChannelId(id)
}
...
func TestBatchSettlementIntegration_DepositThenVoucher(t *testing.T) {
	...
	if firstPayload.Accepted.Scheme != batched.SchemeBatched {
		t.Fatalf("expected scheme %s, got %s", batched.SchemeBatched, firstPayload.Accepted.Scheme)
	}
...
func startBatchedHTTPServer(t *testing.T, pipe *batchedPipeline, route string, price string) (string, func()) {
	...
	// Use the pipeline's pre-built x402Server so the BatchedEvmScheme hooks
...
func TestBatchSettlementIntegration_WithdrawalPendingRefund(t *testing.T) {
	...
	if !strings.EqualFold(batched.NormalizeChannelId(pending[0].ChannelId), channelId) {
