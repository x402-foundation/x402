...
class McpError(Exception):
    """Flat code/data exception used by some transport adapters."""
...
class TestExtractPaymentRequiredFromError:
    ...
    @pytest.mark.parametrize("code", [402, -32042])
    @pytest.mark.parametrize("namespaced", [False, True])
    def test_real_mcp_sdk_error(self, code, namespaced):
        data = {"x402": PAYMENT_REQUIRED_DATA} if namespaced else PAYMENT_REQUIRED_DATA
        error = SDKMcpError(ErrorData(code=code, message="Payment required", data=data))
        result = extract_payment_required_from_error(error)
        assert result is not None
        assert result.accepts[0].amount == "1000"

    @pytest.mark.parametrize("code", [[], {}, "402", 402.0, True, None])
    def test_malformed_error_code_is_not_a_payment_challenge(self, code):
        assert (
            extract_payment_required_from_error({"code": code, "data": PAYMENT_REQUIRED_DATA})
            is None
        )
...
@pytest.mark.parametrize("kind", ["sync", "async", "session"])
@pytest.mark.parametrize("override_seconds", [None, 7])
@pytest.mark.asyncio
async def test_real_sdk_error_retries_once_with_preserved_timeouts(kind, override_seconds):
    from datetime import timedelta

    error = SDKMcpError(
        ErrorData(code=-32042, message="Payment required", data={"x402": PAYMENT_REQUIRED_DATA})
    )
    if kind == "sync":
        transport, payment = MockSyncMCPClient(), MockSyncPaymentClient()
        client = x402MCPClientSync(transport, payment, max_request_timeout_seconds=60)
    else:
        transport, payment = MockAsyncMCPClient(), MockAsyncPaymentClient()
        client = (
            x402MCPSession(transport, payment, max_request_timeout_seconds=60)
            if kind == "session"
            else x402MCPClient(transport, payment, max_request_timeout_seconds=60)
        )
    transport.call_tool.side_effect = [error, MockAsyncMCPResult()]
    kwargs = (
        {"read_timeout_seconds": timedelta(seconds=override_seconds)} if override_seconds else {}
    )
    result = client.call_tool("paid_tool", {"input": "kept"}, **kwargs)
    if kind != "sync":
        result = await result
    assert result.payment_made
    assert transport.call_tool.call_count == 2
    calls = transport.call_tool.call_args_list
    expected_timeout = timedelta(seconds=override_seconds or 60)
    assert calls[0].kwargs["read_timeout_seconds"] == expected_timeout
    assert calls[1].kwargs["read_timeout_seconds"] == expected_timeout
    if kind == "session":
        assert calls[1].kwargs["arguments"] == {"input": "kept"}
        assert "x402/payment" in calls[1].kwargs["meta"]
        assert "resource" not in calls[1].kwargs["meta"]["x402/payment"]
    else:
        assert calls[1].args[0]["arguments"] == {"input": "kept"}
        assert "x402/payment" in calls[1].args[0]["_meta"]
        assert "resource" not in calls[1].args[0]["_meta"]["x402/payment"]
    payment.create_payment_payload.assert_called_once()


@pytest.mark.parametrize("auto_payment", [False, True])
@pytest.mark.asyncio
async def test_session_real_error_disabled_or_nonpayment_never_pays(auto_payment):
    transport, payment = MockAsyncMCPClient(), MockAsyncPaymentClient()
    error = SDKMcpError(
        ErrorData(
            code=-32042 if not auto_payment else -32603, message="error", data=PAYMENT_REQUIRED_DATA
        )
    )
    transport.call_tool.side_effect = error
    client = x402MCPSession(transport, payment, auto_payment=auto_payment)
    with pytest.raises(PaymentRequiredError if not auto_payment else SDKMcpError):
        await client.call_tool("paid_tool", {})
    payment.create_payment_payload.assert_not_called()
    assert transport.call_tool.call_count == 1
