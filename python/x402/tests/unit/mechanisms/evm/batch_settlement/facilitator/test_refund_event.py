"""Refunded-event decode must work with the exported FacilitatorWeb3Signer."""
...
try:
    ...
    from x402.mechanisms.evm.batch_settlement.facilitator.refund import _parse_refunded_amount
...
def test_parse_refunded_amount_with_facilitator_web3_signer():
    ...
    refunded_abi = next(item for item in BATCH_SETTLEMENT_ABI if item.get("name") == "Refunded")
    log = {
        "address": contract_addr,
        "topics": [
            HexBytes(event_abi_to_log_topic(refunded_abi)),
            HexBytes(channel_id),
    ...
    assert _parse_refunded_amount(signer, _Receipt(), contract_addr, channel_id) == "9000"
