...
@pytest.mark.parametrize("version", [0, 3, -1, "2", 2.0, True])
def test_unsupported_version_does_not_reach_facilitator(endpoint, version):
    facilitator = _create_sync_facilitator()
    operation = Mock()
    setattr(facilitator, endpoint[1:], operation)
    body = _make_request_body()
    body["x402Version"] = version
    response = TestClient(create_facilitator_app(facilitator)).post(endpoint, json=body)
    assert response.status_code == 400
    operation.assert_not_called()


@pytest.mark.parametrize("endpoint", ["/verify", "/settle"])
def test_internal_exception_is_not_exposed(endpoint):
    facilitator = _create_sync_facilitator()
    setattr(facilitator, endpoint[1:], Mock(side_effect=RuntimeError("private RPC details")))
    response = TestClient(create_facilitator_app(facilitator)).post(
        endpoint, json=_make_request_body()
    )
...
def test_nonobject_body_is_rejected(sync_client, endpoint, body):
    response = sync_client.post(endpoint, content=json.dumps(body))
    assert response.status_code == 400


@pytest.mark.parametrize("endpoint", ["/verify", "/settle"])
def test_payload_version_must_match_request(sync_client, endpoint):
    body = _make_request_body()
    body["paymentPayload"]["x402Version"] = 1
    assert sync_client.post(endpoint, json=body).status_code == 400


@pytest.mark.parametrize("endpoint", ["/verify", "/settle"])
def test_v1_request_routes_to_registered_v1_scheme(endpoint):
    facilitator = x402FacilitatorSync().register_v1(["base-sepolia"], MockExactScheme())
    body = {
        "x402Version": 1,
        "paymentPayload": {
            "x402Version": 1,
    ...
    assert response.json()["isValid" if endpoint == "/verify" else "success"] is True
